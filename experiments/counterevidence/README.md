...
**B と C は同一形式の JSON を返す。** `counterevidence_documents` の定義文は
`prompts.py` の `_COUNTEREVIDENCE_DEF` を両者が共有しており、片方だけ変更できない。
Refutation Discovery はこのフィールドで採点する。
...
# 手動（課金なし。各プロンプトは新しい会話で実行すること）
python run.py --backend manual --stage 1    # 18 prompts
...
| 7 | `counterevidence_documents` は自己申告。**反証を理解したが申告しない**場合を取りこぼす | 生き残る。ただし B/C で条件は同一 |
