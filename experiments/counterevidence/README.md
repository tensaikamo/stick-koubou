...
- run開始・API呼び出し完了・手動応答ファイル取込のUTC時刻
- dataset / specs / prompts.py / run.py の SHA-256
...
stage 2/3 は入力・コード・prompt SHAのドリフトを拒否する。stage 3 は全6ケースの
A/B/C1/C2応答、C2生成証跡、response SHAが揃うまで `raw.jsonl` を生成しない。
採点器も、provenance欠損、異なるrun/モデル/設定の混在、raw本文とresponse SHAの不一致を
`INCOMPLETE_RESULTS` として拒否する。
