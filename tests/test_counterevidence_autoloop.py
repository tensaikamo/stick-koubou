...
def manifest(calls=None, **changes):
    ...
    value = AUTO.new_manifest("run-001", generator(), judge(), calls or [])
    if changes:
...
def test_judge_call_is_bound_to_a_source_response_sha():
    good = pending_call(
        "J001", role="judge", stage="FINAL_VERDICT", source_response_sha256=digest("source")
    )
    AUTO.validate_manifest(manifest([good]))
    bad = pending_call("J002", role="judge", stage="FINAL_VERDICT")
    with pytest.raises(AUTO.AutoLoopError) as caught:
        manifest([bad])
    assert caught.value.code == "INVALID_PROVENANCE"
...
def test_generator_and_judge_provider_and_session_are_independent(bad_judge):
    with pytest.raises(AUTO.AutoLoopError) as caught:
        AUTO.new_manifest("run-001", generator(), bad_judge, [])
